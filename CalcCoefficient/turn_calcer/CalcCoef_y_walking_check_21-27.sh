...
for target_deg in `seq 21 3 27`
do
	for vel_y in `seq -0.4 0.04 0.4`
	do
		for cog_y in `seq -0.06 0.006 0.06`
		do
			para="$target_x $target_y $target_deg $vel_x $vel_y $cog_x $cog_y 0 1" #0  1 -> 1  1"
...
do
	...
	do
		for cog_y in `seq -0.06 0.006 0.06`
		do
			para="$target_x $target_y $target_deg $vel_x $vel_y $cog_x $cog_y 1 1" #0  1 -> 1  1"
			coef=`./GenerateWalkPattern $para`
			echo "$para" 1>&2
			if [ "$coef" != "$out_of_range" ]; then
				echo $para $coef
