...
int main(int argc, char *argv[])
{
	if (argc != 9 && argc != 10) {
		std::cout << "Generate a walking pattern by using the preview control." << std::endl <<
			"Usage: GenerateWalkPattern <target_x> <target_y> <initial_speed_x> <initial_speed_y> " <<
			"<initial_cog_x> <initial_cog_y> <support leg 0:right 1:left> <0:stop 1:walking> [<filename>]" << std::endl <<
	...
	double speed_x = atof(argv[3]);
	double speed_y = atof(argv[4]);
	double cog_x = atof(argv[5]);
	double cog_y = atof(argv[6]);
	FootStatus foot_status = (atoi(argv[7]) == 1) ? RightLeg : LeftLeg;
	WalkingStatus walking_status = (atoi(argv[8]) == 1) ? Walking : Start;
	if (atoi(argv[8]) >= 2) walking_status = Stop;
	int stop_status = atoi(argv[8]) - 2;	//0:first half,1:second half
	//std::cout << "stop status:" << stop_status << std::endl;
	bool save_file = (argc == 10) ? true : false;
	std::ofstream writing_file;
	if (save_file) {
		writing_file.open(argv[9]);
	...
	plan_node.SetFootStepParameter(MAX_X_STEP, MAX_Y_STEP, 0.0, FOOT_WIDTH, WALKING_HALF_CYCLE);
	plan_node.SetTargetPos(target_x, target_y, 0.0, foot_status, walking_status);
#if 0
	std::cout << "Results of foot steps" << std::endl;
	for (int i = 0; i < plan_node.foot_step_list.size(); i ++)
		std::cout << plan_node.foot_step_list[i] << std::endl;
#endif
...
//	PreviewControl preview_control(SAMPLING_TIME, WALKING_HALF_CYCLE * 3);
	...
	if (save_file){
		...
		gp << "plot" << gp.file1d(xy_pts_refZMP) << "with lines title 'reference ZMP'," << gp.file1d(xy_pts_ZMP) << "with lines title 'ZMP',"  << gp.file1d(xy_pts_COG) << "with lines title 'COG'," << endl;
	}
#endif
#if 1
    if (save_file){
	...
	if (!save_file){
        ...
        fprintf(gp, "set ylabel \"y [m]\"\n");
        //fprintf(gp, "set size ratio -1\n");
        fprintf(gp, "plot '-' with lines lw 5 lt 7 title \"COM\", '-' with lines lw 5 lt 2 title \"CurveFitting\", \n");
