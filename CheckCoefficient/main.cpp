...
/*### 多項式の係数を呼び出し */
    ...
    float stride_x = 0.0f, stride_y = 0.0f;
    float stride_x_gc = 0.0f, stride_y_gc = 0.0f;

    int q = 0;
    int target_deg =0, flag_dx = 0;
...
# if 1
            ...
            if(target[step].deg != 0.0 && flag_dx == 0){
            ...
            stride_x  = trajectory_x*cos(-ref_zmp_th) - (trajectory_y*sin(-ref_zmp_th));
            stride_y  = trajectory_x*sin(-ref_zmp_th) + (trajectory_y*cos(-ref_zmp_th));
#endif
			com = com0;
            // 歩行停止フェーズ出ない場合は以下を実行
			if (walking < 2){
...
#if 1
                    if(flag_dx == 1){//stepが切り替わった最初の場合
                        target_dx = target[step].x - stride_x;
                        stride_x_gc = 0;
                        stride_y_gc = target[step-1].y - target_dy;
                        flag_dx = 2;
                    }else{//step切り替わり以降
                        target_dx = target[step].x - stride_x_gc - stride_x;
                ...
                std::cout << "target_dy  " << to_string(target_dy,2) << std::endl;
                std::cout << "stride_y  " << to_string(stride_y,2) << std::endl;
                std::cout << "ref_zmp_y_gc  "<< ref_zmp_y_gc << std::endl;
                std::cout << "stride_y_gc  " << to_string(stride_y_gc,2) << std::endl;
