...
int main(int argc, char *argv[])
{
    ...
    float start_vel_x = 0.0f, start_vel_y = 0.0f;
    float vel_th = 0.0f;
    ...
    std::vector<std::pair<double,double>> xyth_pts_vel_points_end;
    ...
    std::vector<std::pair<double,double>> current_ref_pos;
    ...
    float trajectory_stride_gc = 0.0f;
...
# if 1
			...
			if (walking < 2){
                ...
                //現在の重心位置
                cog_x   = (new_current_x)*cos(-ref_zmp_th) - (new_current_y)*sin(-ref_zmp_th);//next tarjectory中心の座標系
                cog_y   = (new_current_x)*sin(-ref_zmp_th) + (new_current_y)*cos(-ref_zmp_th);

			} else foot_y = 0;

            if(flag_vel == 0){
                vel_th= ref_zmp_th_gc;
            }
/*
            if(flag_vel == 1){
                vel_x = modi_x;
...
//				printf("%s\n", str);
			...
			if (walking < 3){
                ...
                trajectory_stride_gc += trajectory_stride;
                //STEP 3
...
/*
            std::cout << "ref_zmp_th:  " << rad2deg(ref_zmp_th) << std::endl;
            std::cout << "ref_zmp_th_gc:  " << rad2deg(ref_zmp_th_gc) << std::endl;
            std::cout << "vel_th:  " << rad2deg(vel_th) << std::endl;
*/
		}// 係数計算終了
        ...
        modi_vel_x = v_x;//(v_x-start_vel_x)*cos(vel_th) - (v_y-start_vel_y)*sin(vel_th)+start_vel_x;
        modi_vel_y = v_y;//(v_x-start_vel_x)*sin(vel_th) + (v_y-start_vel_y)*cos(vel_th)+start_vel_y;
...
//            modi_acc_y = (acc_x)*sin(ref_zmp_th_gc) + (acc_y)*cos(ref_zmp_th_gc);
        ...
        if(i%34 == 1){
            ...
            xyth_pts_vel_points_end.push_back(std::make_pair(modi_vel_x, modi_vel_y ));
        ...
        xyth_pts_vel.push_back(std::make_pair(v_x, v_y));
        xyth_pts_acc.push_back(std::make_pair(acc_x, acc_y));
...
// 確認用　描画
    ...
    for(std::size_t i=0;i<xyth_pts_vel_points_end.size();i++) fprintf(gp0, "%f\t%f\n", (i*0.34), xyth_pts_vel_points_end[i].first ); fprintf(gp0, "e\n");
    fprintf(gp0,"exit\n");
    pclose(gp0);
#endif
#if 1
    ...
    for(std::size_t i=0;i<xyth_pts_vel_points_end.size();i++) fprintf(gp0, "%f\t%f\n", (i*0.34), xyth_pts_vel_points_end[i].second ); fprintf(gp1, "e\n");
